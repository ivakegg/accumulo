...
abstract class TabletGroupWatcher extends Daemon {
  ...
  private void repairMetadata(Text row) {
    ...
    try {
      ...
      if (future.size() > 0 && assigned.size() > 0) {
        ...
        Master.log.info("Attempted a repair, but nothing seems to be obviously wrong. " + assigned
            + " " + future);
        return;
      }
      ...
      Master.log.error("Metadata table is inconsistent at " + row
          + " and all assigned/future tservers are still online.");
    } catch (Throwable e) {
    ...
    if (tls.extent.overlaps(range)) {
      for (Text splitPoint : new Text[] {range.getPrevEndRow(), range.getEndRow()}) {
        ...
        try {
          ...
          if (conn != null) {
            Master.log
                .info("Asking " + tls.current + " to split " + tls.extent + " at " + splitPoint);
  ...
  private void deleteTablets(MergeInfo info) throws AccumuloException {
    ...
    try {
      ...
      if (followingTablet != null) {
        Master.log
            .debug("Updating prevRow of " + followingTablet + " to " + extent.getPrevEndRow());
  ...
  private void flushChanges(SortedMap<TServerInstance,TabletServerStatus> currentTServers,
    ...
    if (!assignedToDeadServers.isEmpty()) {
      ...
      Master.log.debug(assignedToDeadServers.size() + " assigned to dead servers: "
        ...
        if (unassigned.containsKey(assignment.getKey())) {
          if (assignment.getValue() != null) {
            if (!currentTServers.containsKey(assignment.getValue())) {
              Master.log.warn("balancer assigned " + assignment.getKey()
                  + " to a tablet server that is not current " + assignment.getValue()
                  + " ignoring");
              continue;
            }
